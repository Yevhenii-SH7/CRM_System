...
interface ProjectsProps {
  ...
  onProjectViewed?: () => void; // Add callback function
}
...
function Projects({ projectIdToView, onProjectViewed }: ProjectsProps) {
  ...
  // Handle automatic project viewing when projectIdToView is provided
  useEffect(() => {
    if (projectIdToView) {
      const project = projects.find(p => p.id === projectIdToView);
      if (project) {
        setIsViewingProject(project);
        // Notify parent that project has been viewed
        onProjectViewed?.();
      }
    }
